...
class Anarchy extends BaseMapGenerator
{
    ...
    public function runGenerator()
    {
        ...
        $cellProcessor = new CellProcessing($this->map, new TileProcessing($this->map));
        $this->setCellProcessor($cellProcessor);
        ...
        $seed = $this->getSeed();
        if (empty($seed)) {
            $seed = sha1(md5(microtime(true)));
            $this->setSeed($seed);
        }

        // Build height-map noise maker seeded for stability.
        $perlinHeightMap = new NoiseMaker($seed);
        ...
        // Use a reversed-seed noise maker to diversify trees.
        $treeSeed = strrev($seed);
        $perlinTreeMap = new NoiseMaker($treeSeed);
        $this->setTreeNoiseMaker($perlinTreeMap);
        $this->setupTreeArrGrid($gridsize);

        // Populate height map and tree overlays.
        $this->makeNoiseForHeightMap($arrMap, $perlinHeightMap, $gridsize);
        $this->doTreeLoops($arrMap, $gridsize - 1, $perlinTreeMap);

        // First ridge pass: classify cells based on height + tree data.
        $cellProcessor->saveToDatabaseWhileProcessing();
        $cellProcessor->processCellsFromHeightMap($arrMap);

        // Optional second ridge pass: raise the mountain line slightly and reprocess existing mountain cells.
        $originalMountainLine = $cellProcessor->getMountainLine();
        $cellProcessor->setMountainLine($originalMountainLine + 12);
        $cellProcessor->processMountainCells();
        // Restore original threshold for downstream processors.
        $cellProcessor->setMountainLine($originalMountainLine);
    }
