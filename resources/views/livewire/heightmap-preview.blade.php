<div class="p-4">
  <div class="flex items-center gap-4 mb-2">
    <div>
      ...
      <span class="text-sm">{{ $mapId }}</span>
    </div>
    <div>
      <span class="font-semibold">Grid:</span>
      <span class="text-sm">{{ $sizeX }} × {{ $sizeY }}</span>
    </div>
  </div>

  <div class="border rounded p-2 bg-gray-50">
    <canvas id="heightmapCanvas" class="w-full" style="image-rendering: pixelated;" width="{{ $sizeX }}" height="{{ $sizeY }}"></canvas>
  </div>

  <script>
    (function() {
      const mapId = JSON.parse('@json($mapId)');
